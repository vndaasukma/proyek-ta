...
<div class="container-fluid py-4">
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-header bg-white border-0 py-3">
            <h5 class="m-0 fw-bold text-success text-lowercase">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.fasilitas.store') }}" method="POST" enctype="multipart/form-data" class="mb-5 p-4 bg-light rounded-4 border">
                ...
                <div class="row g-3 align-items-end">
                    <div class="col-md-3">
                        <label class="small fw-bold text-lowercase text-muted mb-1">nama fasilitas</label>
                        <input type="text" name="nama_fasilitas" class="form-control rounded-3" placeholder="misal: green house a" required>
                    </div>
                    <div class="col-md-4">
                        <label class="small fw-bold text-lowercase text-muted mb-1">deskripsi</label>
                        <input type="text" name="deskripsi" class="form-control rounded-3" placeholder="deskripsi singkat fasilitas" required>
                    </div>
                    <div class="col-md-3">
                        <label class="small fw-bold text-lowercase text-muted mb-1">foto fasilitas</label>
                        <input type="file" name="gambar" class="form-control rounded-3" required>
                    </div>
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-success w-100 rounded-3 fw-bold text-lowercase">simpan</button>
                    </div>
                </div>
            </form>
                ...
                <div class="col-md-3">
                    <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                        ...
                        <div class="card-body p-3">
                            <h6 class="fw-bold text-lowercase mb-1">{{ $f->nama_fasilitas }}</h6>
                            ...
                            <form action="{{ route('admin.fasilitas.destroy', $f->id) }}" method="POST" onsubmit="return confirm('Hapus fasilitas ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline-danger w-100 rounded-pill text-lowercase">
